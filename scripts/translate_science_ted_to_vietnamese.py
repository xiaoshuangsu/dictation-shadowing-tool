...
def main():
    ...
    # 获取 Science and Facts 和 TED演讲 素材
    client = create_client(SUPABASE_URL, SUPABASE_SERVICE_KEY)

    result1 = client.table('materials').select('id, title, category').eq('category', 'Science and Facts').execute()
    result2 = client.table('materials').select('id, title, category').eq('category', 'TED演讲').execute()

    materials = result1.data + result2.data

    print("="*100)
    print("🔬 批量翻译科学/TED类素材为越南语（学术风格）")
    print("="*100)
    print(f"\n📊 总素材数: {len(materials)}")
    print(f"📝 翻译风格: 科学/TED频道（术语严谨 + 正式表达）")
    ...
    for batch_idx in range(total_batches):
        ...
        for idx, material in enumerate(batch):
            ...
            if not result.data:
                print(f"❌ [{current_num}/{len(materials)}] 未找到: {title[:50]}")
            ...
            if has_vi:
                print(f"[{current_num}/{len(materials)}] ⏭️  已有翻译: {title[:60]}")
                stats['success'].append(title)
                continue

            # 处理翻译
            print(f"[{current_num}/{len(materials)}] 🎬 {title[:60]}")
        ...
        if batch_idx < total_batches - 1:  # 最后一批不需要 commit
            try:
                commit_msg = f"feat(vi): 翻译科学/TED素材 batch {batch_idx + 1}/{total_batches} ({len(batch)} materials)"
    ...
    # 保存统计结果
    with open('/tmp/science_ted_translation_stats.json', 'w') as f:
