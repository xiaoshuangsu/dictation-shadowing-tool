...
"""
IELTS 词汇抓取脚本 - V3.0
完整版：IELTS 词汇 + 19 国翻译 + Edge TTS + R2 存储

功能：
1. IELTS 词汇数据源（Cambridge IELTS 听力测试）
2. 19 国语言翻译
3. Edge TTS 音频生成
4. R2 持久化存储
...
使用方法：
  # 测试模式（5 个单词）
  python scripts/prepopulate_ielts_v3.py --test --ielts

  # 全量运行
  python scripts/prepopulate_ielts_v3.py --ielts
...
class IELTSScraper:
    ...
    从 engnovate.com 抓取 IELTS 词汇数据（Cambridge IELTS 听力测试）
    """

    # IELTS 分类页面 URL
    CATEGORY_URL = "https://engnovate.com/flashcards/?category=ielts"
    ...
    def __init__(self, max_words: Optional[int] = None):
        ...
        logger.info("📚 IELTSScraper 初始化完成")
        if max_words:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='IELTS 词汇抓取脚本 V3.0',
        formatter_class=argparse.RawDescriptionHelpFormatter
    )
    parser.add_argument('--test', action='store_true', help='测试模式（5 个单词）')
    parser.add_argument('--ielts', action='store_true', help='使用 IELTS 词汇数据源')
    parser.add_argument('--limit', type=int, default=None, help='单词数量限制（默认不限制）')
    ...
    # 处理参数逻辑
    max_words = args.limit
    if args.test and max_words is None:
        max_words = 5

    print("=" * 70)
    print("IELTS 词汇抓取脚本 V3.0")
    print("=" * 70)
    print()
    ...
    # 正常模式（翻译 + 音频）
    # 创建 IELTS 抓取器
    scraper = IELTSScraper(max_words=max_words)
