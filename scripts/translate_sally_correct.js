...
// 翻译函数（使用智谱 GLM）
async function translateText(text, fromLang = 'zh', toLang = 'zh_hant') {
  const langMap = {
    'zh_hant': '繁體中文',
    'vi': '越南語'
  };
...
async function updateTranslations() {
  try {
    ...
    for (const sentence of sentencesToTranslate) {
      const translation = await translateText(sentence, 'zh', 'zh_hant');
      if (translation) {
        console.log(`"${sentence}" => "${translation}"`);

        // 更新 transcript
        transcript.forEach((segment) => {
          if (segment.translation?.zh === sentence) {
    ...
    console.log('\n=== 翻译成越南语 ===');
    for (const sentence of sentencesToTranslate) {
      const translation = await translateText(sentence, 'zh', 'vi');
      if (translation) {
        console.log(`"${sentence}" => "${translation}"`);
