...
def main():
    ...
    # 加载素材列表
    with open('/tmp/ielts_materials.json', 'r') as f:
        materials = json.load(f)

    client = create_client(SUPABASE_URL, SUPABASE_SERVICE_KEY)

    print("="*100)
    print("🎓 批量翻译 IELTS 素材为越南语（学术风格）")
    print("="*100)
    print(f"\n📊 总素材数: {len(materials)}")
    print(f"📝 翻译风格: IELTS/学术频道（正式代词 + 专业术语）")
    ...
    for batch_idx in range(total_batches):
        ...
        for idx, material in enumerate(batch):
            ...
            process_result = process_material(
                ...
                title,
                category,
                full_material.get('difficulty', 'C1'),
                transcript,
                client
            )
        ...
        if batch_idx < total_batches - 1:  # 最后一批不需要 commit
            try:
                commit_msg = f"feat(vi): 翻译 IELTS 素材 batch {batch_idx + 1}/{total_batches} ({len(batch)} materials)"
    ...
    # 保存统计结果
    with open('/tmp/ielts_translation_stats.json', 'w') as f:
