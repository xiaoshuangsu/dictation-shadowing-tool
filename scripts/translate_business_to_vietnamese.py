...
def main():
    ...
    # 职场类素材列表（根据标题或内容判断）
    # 当前没有明确的 category='business'，通过关键词匹配
    ...
    for material in result.data:
        ...
        # 职场商务类关键词
        business_keywords = [
            'corruption', 'bribe', 'business', 'office', 'professional',
            'workplace', 'company', 'corporate', 'kickback'
        ]
        if any(kw in title for kw in business_keywords):
            business_materials.append(material)

    print("="*100)
    print("💼 批量翻译职场商务类素材为越南语（正式风格）")
    print("="*100)
    print(f"\n📊 总素材数: {len(business_materials)}")
    print(f"📝 翻译风格: 职场正式（用词正式 + 语气中性 + 专业术语）")
    print(f"📦 批次大小: 10 个素材/commit")
    print("="*100)
    print()

    if not business_materials:
        print("❌ 未找到职场商务类素材")
        return
    ...
    # 分批处理
    batch_size = 10
    total_batches = (len(business_materials) + batch_size - 1) // batch_size

    for batch_idx in range(total_batches):
        start_idx = batch_idx * batch_size
        end_idx = min(start_idx + batch_size, len(business_materials))
        batch = business_materials[start_idx:end_idx]
        ...
        for idx, material in enumerate(batch):
            ...
            category = material.get('category', 'business')

            # 获取完整素材信息
            result = client.table('materials').select('*').eq('id', material_id).execute()
            if not result.data:
                print(f"❌ [{current_num}/{len(business_materials)}] 未找到: {title[:50]}")
            ...
            if has_vi:
                print(f"[{current_num}/{len(business_materials)}] ⏭️  已有翻译: {title[:60]}")
                stats['success'].append(title)
                continue

            # 处理翻译（使用 business 分类以应用正式风格）
            print(f"[{current_num}/{len(business_materials)}] 🎬 {title[:60]}")
            process_result = process_material(
                material_id,
                title,
                'business',  # 强制使用 business 分类
        ...
        if batch_idx < total_batches - 1:  # 最后一批不需要 commit
            try:
                commit_msg = f"feat(vi): 翻译职场商务素材 batch {batch_idx + 1}/{total_batches} ({len(batch)} materials)"
    ...
    # 保存统计结果
    with open('/tmp/business_translation_stats.json', 'w') as f:
