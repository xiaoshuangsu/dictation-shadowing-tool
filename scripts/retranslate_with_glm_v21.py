#!/usr/bin/env python3
"""
专业级上下文感知翻译脚本（V21.0 - 字幕级对齐版）
使用 GLM-4 API 生成地道、具备上下文理解能力的翻译
新增：字幕级对齐规则（独立块原则 + 强制索引前缀）
解决：GLM-4 合并翻译问题（断句导致的对齐失败）
"""
...
1️⃣ 独立块原则 (Atomic Block Rule)：
- ⚠️ 将每一行视为独立的"意义碎片"
- ⚠️ 禁止重组：严禁为了中文语序通顺而将第 N+1 行的主语挪到第 N 行
- ⚠️ 每行翻译必须保持独立，哪怕在中文里只是个未完成的短语

翻译示例：
  原 [Line 1]: Once upon a time, in a small village
  译 [Line 1]: 从前，在一个小村庄里，

  原 [Line 2]: surrounded by mountains, lived a young man named Marco.
  译 [Line 2]: 群山环绕的地方，住着一个名叫马可的年轻人。

2️⃣ 合并句处理规则（重要）：
- ⚠️ 如果原文包含对话后紧跟动作描述（如："Can you help me?" The old man smiled.）
- ⚠️ 必须整句翻译，不能拆分成两个翻译
- 正确示例：
  原 [Line 1]: "Can you help me?" The old man smiled.
  译 [Line 1]: "你能帮我吗？"老人微笑着。
- ❌ 错误：只翻译前半句或拆分成两句

3️⃣ 强制索引前缀：
- 输入每行带有 [Line N] 前缀
- 输出必须也带有相同的 [Line N] 前缀
- 这强迫模型意识到这是独立任务

4️⃣ 返回格式要求：
- 必须返回：{"translations": ["[Line 1] 翻译1", "[Line 2] 翻译2", ...]}
- 每个翻译前面必须保留 [Line N] 前缀
- 脚本会自动清理前缀，但返回时必须带有
...
- Come on (不相信/嘲讽语气) → "得了吧"、"别逗了"、"别装了"（❌ "快点儿"、"来吧"）
...
- certain → "笃定"（❌ "确信"）
- restless → "心神不宁"或"焦躁"（❌ "不安分"、"不安"）
- uneasy → "不安"或"焦虑"（❌ 严禁重复使用"心神不宁"）
- psychologist → "心理学家"（❌ "心里学家"）

【体育术语保护】（冰球/运动类必须遵守）：
- Body check (冰球) → "身体冲撞"（❌ "身体检查"）
- Checking (冰球动作) → "冲撞"（❌ "检查"）
- Puck (冰球) → "冰球"（❌ "球"）
- Rink (冰球场) → "冰球场"（❌ "场地"）
- icing (冰球术语) → "死球"（❌ "结冰"）
...
【输出格式】：
⚠️ 必须返回格式：{"translations": ["[Line 1] 翻译1", "[Line 2] 翻译2", ...]}
⚠️ 每个翻译前面必须保留 [Line N] 前缀
⚠️ 如果输入 4 句，必须返回 4 个翻译，不能多也不能少！
❌ 严禁在翻译内容中使用其他方括号 [ ]，只在开头保留 [Line N] 前缀
"""
...
def detect_and_split_merged_sentences(texts: List[str]) -> List[str]:
    """
    检测并拆分合并的句子（如对话后紧跟动作描述）
    ...
    if merge_count > 0:
        print(f"      🔍 检测到 {merge_count} 处合并句，已拆分")
...
def translate_batch(texts: List[str], video_title: str, category: str, difficulty: str, strict_mode: bool = False) -> Optional[List[str]]:
    ...
    参数:
        strict_mode: 严格模式（用于重试，更强制的断句要求）
    """
    ...
    # 根据是否严格模式选择不同的提示语
    if strict_mode:
        user_content = f"""⚠️ 严格模式：检测到合并翻译，请按照以下要求重新翻译：
...
🚨 严格对齐要求：
1. 每行 [Line N] 必须有对应的翻译，哪怕中文翻译是不完整的短语
2. 禁止将相邻两行合并为一行翻译
3. 返回格式：每个翻译必须带有 [Line N] 前缀
...
- 必须返回：{{"translations": ["[Line 1] 翻译1", "[Line 2] 翻译2", ...]}}
- 每个翻译前面必须保留 [Line N] 前缀
- 行数必须与原文一致：{len(texts)} 行
"""
    else:
        user_content = f"""请翻译以下 {len(texts)} 行字幕：
...
5. certain 翻译为"笃定"
6. ⚠️ uneasy 翻译为"不安"或"焦虑"，**严禁**重复使用"心神不宁"（restless 专用）
7. ⚠️ psychologist 翻译为"心理学家"（❌ "心里学家"）
...
def process_material(material_id: str, video_title: str, category: str, difficulty: str, transcript: List[Dict], supabase_client) -> Dict:
    ...
    batch_size = 4  # V21: 减小批次大小以提高对齐成功率
    geo_fixes = 0
    ...
    for i in range(0, len(texts), batch_size):
        ...
        for retry in range(3):  # 最多尝试 3 次
            ...
            if translations is None:
                if retry < 2:
                    mode_str = "严格模式" if strict_mode else "正常模式"
                    print(f" [{mode_str} 重试 {retry+1}/2]...", end="", flush=True)
                    time.sleep(0.5)
                    continue
                else:
                    print(f" ❌ 对齐失败")
            ...
            if validate_alignment(len(batch_texts), translations):
                ...
                if retry < 2:
                    print(f" [对齐失败，严格模式重试 {retry+1}/2]...", end="", flush=True)
                    time.sleep(0.5)
                    translations = None
                    continue
                else:
...
def main():
    ...
    print("="*100)
    print("🌍 专业级上下文感知翻译脚本 V21.0 - 字幕级对齐版")
    print("="*100)
    print(f"\n📋 模式: {MODE}")
    ...
    if MODE == "single":
        ...
        print(f"\n🚀 全量 Auto-pilot 模式")
        print(f"="*100)
        print(f"[模式]：字幕级对齐 + 原子提交 + 实时汇报")
        print(f"[批次大小]：4 句/批（减小以提高对齐成功率）")
        print(f"="*100)
