...
V19.5 版本实现了**根据素材分类自动选择翻译风格**的能力，解决了之前"一刀切"导致的翻译质量问题。

### 🎭 分类风格规则（三档适配）

#### 1️⃣ 科学科普类（Science/TED-Ed）
**风格特征**：术语严谨，不带个人情绪，无语气词

**示例对照**：
| 英文 | ❌ 错误翻译 | ✅ 正确翻译 |
|------|-----------|-----------|
| Well, for one thing, you'd have only a month to live. | 哎呀，首先你只能活一个月啦！ | 首先，你将只剩下一个月的时间来生活。 |
| This tug of war keeps us in orbit. | 这场拉锯战把我们保持在轨道上呢。 | 这场拉锯战使我们保持在轨道上。 |
| Our planet is no more. | 我们的星球已经不在了。 | 我们的星球已经不复存在。 |

**风格要求**：
- ✅ 术语准确：orbit → 轨道，planet → 星球
- ✅ 客观陈述：无主观词汇（我觉得、好像、似乎）
- ✅ 无语气词：禁止使用"啊、呢、吧、嘛、哈"

---

#### 2️⃣ 职场正式类（Business/Culture）
**风格特征**：用词正式，不使用口语俚语，语气中性

**示例对照**：
| 英文 | ❌ 错误翻译 | ✅ 正确翻译 |
|------|-----------|-----------|
| How can corruption be stopped? | 怎么才能阻止腐败啊？ | 如何制止腐败？ |
| government sometimes decide which company should build a road | 政府有时候会决定哪个公司修路。 | 例如，政府有时会决定哪家公司应该修建道路。 |
| A kickback is similar to a bribe | 回扣跟贿赂差不多。 | 回扣与贿赂类似，区别在于... |

**风格要求**：
- ✅ 用词正式：corruption → 腐败，bribe → 贿赂，kickback → 回扣
- ✅ 无口语俚语：禁止"逗我、拿我开涮、忽悠、上当"
- ✅ 语气中性：无夸张表达（！！、！！！）

---

#### 3️⃣ 日常生活类（Daily Life/Conversation）
**风格特征**：口语俚语 + 强制语气词，B 站/短视频风格

**示例对照**：
| 英文 | ❌ 词典中文 | ✅ 地道口语 |
|------|-----------|-----------|
| You were joking? | 你在开玩笑吗？ | 你逗我呢？ |
| You were pulling my leg that whole time? | 你一直都在拉我的腿？ | 你一直都在拿我开涮啊？ |
| I got you, though. Didn't I? | 我捉到你了，对吧？ | 嘿嘿，上当了吧？是不是？ |
| Thanks for saying those nice things about me, though. | 虽然你那么夸我，我还是很高兴。 | 不过，谢啦，难得听你这么夸我。 |

**口语终极准则**：
1. **拒绝"词典中文"**：
   - You were joking? → "你逗我呢？"（❌ "你在开玩笑吗？"）
   - I got you! (整蛊) → "嘿嘿，上当了吧？"（❌ "我捉到你了"）
   - Pulling my leg → "拿我开涮"或"忽悠我"（❌ "拉我的腿"）

2. **语气词强制要求**：
   - 非正式口语必须包含："啊、呢、吧、嘛、哩、哈"
   - 没有语气词的口语翻译一律判定为失败

3. **情绪对齐**：
   - 整蛊成功 → 调侃 + 得意（"嘿嘿，上当了吧？"）
   - 被骗反应 → 恍然大悟 + 难以置信（"我都不敢信了！"）
...
#### 长句拆分规则
**句型**：that comes when 结构
- ❌ 错误：定语堆在名词前（"你确信自己能力时的乐观"）
- ✅ 正确：使用"源于...所带来的..."结构
- 示例：optimism that comes when you are certain → "源于笃定自身能力所带来的乐观"

#### 引导句处理规则
**句型**：Take/Add + 抽象名词 + that...
- ❌ 错误：死板单词堆砌（"拥有你是...的信念"）
- ✅ 正确：使用"采取这一信念："结构
- 示例：Take the belief that you are valuable → "采取这一信念：你是有价值、值得且有能力的。"

#### 不完整句处理规则
**原则**：如果原文以逗号结尾或语义不完整，中文翻译也要保持不完整
- 示例：When faced with a big challenge where potential failure seems to lurk at every corner,
  → 面对重大挑战，似乎失败隐藏在每个角落时，（✅ 保持未完成状态）
...
| `claude code guide.md` | 本文档，包含完整翻译规范 |
...
#### 职场类检查清单
- [ ] 无口语俚语（逗我、拿我开涮、忽悠、上当）
- [ ] 用词正式（corruption → 腐败，非"贪腐"）
- [ ] 语气中性（无夸张感叹）

#### 日常生活类检查清单
- [ ] 有语气词（至少包含：啊、呢、吧、嘛、哈之一）
- [ ] 拒绝词典中文（无"你在开玩笑吗"、"捉到"）
- [ ] 口语俚语正确（pulling leg → 开涮，I got you → 上当）
- [ ] 情绪对齐（整蛊 → 调侃，感谢 → 亲切）
...
#### 单个素材翻译
```bash
MODE=single SINGLE_ID=<material-id> python3 scripts/retranslate_with_glm_v19.py
```

#### 演示模式（5 个素材）
```bash
MODE=demo python3 scripts/retranslate_with_glm_v19.py
```

#### 全量翻译
```bash
MODE=full python3 scripts/retranslate_with_glm_v19.py
```
...
| 素材 | 分类 | 关键句子 | 修复前 | 修复后 | 状态 |
|------|------|---------|--------|--------|------|
| April Fool's | 日常生活 | I got you, though. | 我捉到你了。 | 嘿嘿，上当了吧？是不是？ | ✅ |
| April Fool's | 日常生活 | Pulling my leg | 你拉我的腿？ | 你一直都在拿我开涮啊？ | ✅ |
| What If | 科学 | you'd have only a month to live. | 哎呀，你只能活一个月啦！ | 你将只剩下一个月的时间来生活。 | ✅ |
| Corruption | 职场 | How can corruption be stopped? | 怎么才能阻止腐败啊？ | 如何制止腐败？ | ✅ |

---

### 🎯 后续优化方向

1. **更细化的分类**：增加"艺术类"、"新闻类"等特定风格
2. **上下文窗口扩大**：前后各 5 句，而不仅仅是当前批次
3. **翻译质量评分**：建立自动评分机制，筛选低质量翻译
4. **用户反馈闭环**：收集用户对翻译的反馈，持续优化 Prompt

---

## 🐛 常见翻译陷阱与修复 (V19.6)

### 问题 1：反问句式对齐错误 (Tag Questions)

**症状**：
- `I got you, though. Didn't I?` → `嘿嘿，上当了吧？没吧？` ❌
- `...right?` → `...对不对？` ❌

**原因**：模型将确认语气的反问句直译为否定形式

**规则**：
```
遇到 "...didn't I?" 或 "...right?" 类确认语气：
  ❌ 严禁直译为"不是吗？"、"没吧？"、"对不对？"
  ✅ 整蛊语境：译为"是不是？"、"对吧？"或合并到前句
```
...
### 问题 2：指代对象错误 (Reference Check)

**症状**：
- `Especially that you think I'm good looking.` → `尤其是你笃定我觉得我长得好看。` ❌

**原因**：
1. "笃定"是书面词，不适合整蛊对话
2. 指代对象错误：`you think I'm` = "你觉得我"，不是"我觉得我"

**规则**：
```
指代对象校验：
- "you think I'm..." → "你觉得我..."（主语是 you，宾语是 I）
- 整蛊对话禁用书面词"笃定"，替换为"居然"、"竟然"
```

**修复后**：
- `Especially that you think I'm good looking.` → `尤其是你居然还说我长得帅。` ✅
...
B3L4 Dialogue 素材翻译存在角色倒置和逻辑错误。

### 新增规则

#### 1. 角色追踪 (Role Tracking)
...
#### 2. 动词语境分析 (Semantic Disambiguation)

**问题**：物品所属关系判断错误

| 英文 | ❌ 错误 | ✅ 正确 |
|------|--------|--------|
| I called her for my keys. | 我打电话找她借钥匙。 | **我打电话问她钥匙的事。** |
...
#### 3. 情绪对齐 (Tone Alignment)

**问题**：使用了"战斗"、"攻击"等死板词汇

| 英文 | ❌ 错误 | ✅ 正确 |
|------|--------|--------|
| attacked you | 攻击你了 | **扑向你了** |
| fought the ghost | 与鬼战斗 | **跟那个鬼斗了半天** |

**规则**：
```
对话类素材严禁使用"战斗"、"攻击"这种死板的词：
- attacked you → "扑向你"
- fought the ghost → "跟那个鬼斗了半天"
```

---

#### 4. 去辞海化 (Natural Expression)
...
B3L8 Dialogue 素材翻译存在术语不准确和句型误译。

### 新增规则

#### 1. 特定术语保护 (Geographic Terms)
...
#### 2. 气候询问句型 (Climate Questions)

**问题**：误译为询问人的状态

| 英文 | ❌ 错误 | ✅ 正确 |
|------|--------|--------|
| What's your winter like? | 你冬天怎么样？ | **你们那儿的冬天是什么样的？** |
...
#### 3. 情感反馈自然化 (Natural Feedback)

**问题**：使用生硬的直译

| 英文 | ❌ 错误 | ✅ 正确 |
|------|--------|--------|
| How interesting! | 真有趣 | **真新鲜！** |
| How different! | 真不同 | **反差真大！** |

**规则**：
```
情感反馈应自然地道：
- How interesting! → "真新鲜！"或"真有意思！"（❌ "真有趣"）
...
Empty Your Mind 素材翻译缺乏文学化表达，使用了过于直白的词汇。

### 新增规则

#### 1. 关键词替换 (Keywords Replacement)

**问题**：直译导致语气不当

| 英文 | ❌ 错误 | ✅ 正确 |
|------|--------|--------|
| restless | 不安分 | **心神不宁/焦躁** |
| thoughts（内心） | 想法 | **杂念/念头** |
| In this moment | 在这个时刻 | **当下/此时此刻** |
...
#### 3. 保持简洁 (Concise & Powerful)

**规则**：
```
励志故事语言短促有力：
- ✅ Marco was very restless. → "马可感到心神不宁。"
- ❌ Marco was a very restless person. → "马可是一个非常不安分的人。"
```
