...
- **后端：幂等性（Idempotency）与 Streak 更新** 🧠
  - **状态预检**：API 在执行 UPDATE 前检查 `mastery_status`
  - **重复请求拦截**：如果请求状态与数据库一致，直接返回 200 OK，不执行写入
  - **消除 500 错误**：通过幂等性校验，避免并发写入冲突

  - **Streak 即时达标逻辑**：
    - **触发条件**：`reviewed_today >= 20` 且 `last_streak_update` 不是今天
    - **实时反馈**：`current_streak` 立即 +1，前端通过 API 响应实时获取最新天数
    - **效果**：实现 3 天变 4 天的即时反馈，无需等待次日刷新

### Technical
- **关键文件变更清单** 📝
  - `ReviewOverlay.tsx`：重构 `handleNext` 函数，引入 `useRef` 同步锁
  - `api/user-words/route.ts`：增加幂等性校验，集成 Streak 实时计算逻辑
  - `VocabularyHubContent.tsx`：优化 Today's Review 的计算源，确保不受 SWR 异步刷新干扰
...
### Fix
- **修复 API 400 错误并清理调试日志** 🎯
  - 成功解决了复习功能中的 API 400 错误问题，现在点击 "Too Easy" 或 "Kinda Know" 按钮能正确更新复习进度。
  - 清理了冗余的调试日志，只保留关键错误捕获逻辑，提升代码整洁度和可维护性。
...
### Fix
- **素材原句对齐逻辑重构 (Precision Alignment)** 🎯
  - **问题描述**：修复了单词（如 hurtle）素材原句显示偏移的问题。原逻辑仅依赖时间戳，导致在字幕密集区域会匹配到相邻的错误句子。

  - **修复方案**：
    - **内容包含性校验**：后端在匹配时间戳后，会强制检查句子文本是否包含目标单词。
    - **滑动窗口检索**：若当前句子不匹配，自动在前后 1-3 秒范围内搜索候选句，确保 100% 精准对齐。
    - **前后端同步**：统一了列表页与闪卡背面的数据源，确保展示逻辑一致。

  - **修改文件**：
    - `src/app/api/user-words/route.ts` - GET 路由匹配逻辑增强

- **线上环境稳定性修复 (Emergency Fix for 500 Error)** 🚨
  - **问题原因分析**：
    - **数据库死锁/超时**：原代码在 GET 请求循环内部同步执行了 `supabase.update()` 操作。当单词量较多时，密集的写操作导致请求超时并引发生产环境 500 错误。
    - **动态路由冲突**：由于使用了请求头数据，未显式声明 `force-dynamic` 导致 Vercel 构建部署异常。

  - **修复动作**：
    - **移除同步写入**：将 GET 请求中的自动修复（auto-fix）数据库写入逻辑移除，改为"只读匹配"以确保响应速度。
    - **部署补丁**：在 user-words 相关 API 路由中强制添加 `export const dynamic = 'force-dynamic'`。
    - **容错处理**：为匹配逻辑增加 Try-Catch 保护，防止单个单词数据异常导致整个 API 崩溃。

  - **修改文件**：
    - `src/app/api/user-words/route.ts` - 移除 auto-fix 逻辑，添加异常保护
