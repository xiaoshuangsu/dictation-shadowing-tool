  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (objectKey.endsWith('.jpg') || objectKey.endsWith('.jpeg') ||
          objectKey.endsWith('.png') || objectKey.endsWith('.webp')) {
      ...
      headers.set('X-Worker-Version', 'v2.2-' + Date.now()); // 版本标识，便于调试

      // HEAD 请求：只返回头信息
      if (request.method === 'HEAD') {
        const headHeaders = new Headers(headers);
        headHeaders.set('Content-Length', object.size.toString());
        return new Response(null, { headers: headHeaders, status: 200 });
      }
      ...
      if (rangeHeader) {
        console.log('Range header detected:', rangeHeader);
        ...
        if (match) {
          ...
          console.log('Range parsed:', { start, end, objectSize: object.size });
          ...
          console.log('Fetching R2 range:', { offset: start, length: end - start + 1 });
          ...
          console.log('R2 range result:', rangedObject ? 'success' : 'null');

          if (rangedObject) {
            console.log('Returning 206 response');
      ...
      finalHeaders.set('Content-Length', object.size.toString());
      return new Response(object.body, { headers: finalHeaders });
